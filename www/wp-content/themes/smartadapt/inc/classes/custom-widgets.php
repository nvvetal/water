...
class WP_Widget_Feeds_smartadapt extends WP_Widget {

	function __construct() {
		$widget_ops = array( 'classname' => 'smartlib_widget_feeds', 'description' => __( "Feeds icons", 'smartadapt' ) );
		parent::__construct( 'widget_feeds', __( 'SmartAdapt: Follow us', 'smartadapt' ), $widget_ops );
	}

	function widget( $args, $instance ) {
		extract( $args );
		$title = apply_filters( 'widget_title', empty( $instance['title'] ) ? '' : $instance['title'], $instance, $this->id_base );
		$checked_rss = empty( $instance['checked_rss'] ) ? '' : $instance['checked_rss'];
		$checked_atom = empty( $instance['checked_atom'] ) ? '' : $instance['checked_atom'];
		$checked_feed = empty( $instance['checked_feed'] ) ? '' : $instance['checked_feed'];
			...
			<ul class="smartlib-feed-list">
				...
				if($checked_rss){
					...
					<li class="smartlib-rss-link">
						<a href="<?php bloginfo('rss2_url'); ?>" title="<?php _e( 'RSS url', 'smartadapt' ); ?>" target="_blank"><?php _e( 'RSS url', 'smartadapt' ); ?></a>
					</li>
						<?php
				}
				?>
				...
				if($checked_atom){
					...
					<li class="smartlib-atom-link">
						<a href="<?php bloginfo('atom_url'); ?>" title="<?php _e( 'Atom url', 'smartadapt' ); ?>" target="_blank"><?php _e( 'Atom url', 'smartadapt' ); ?></a>
					</li>
					<?php
				}
				?>
...
// Use current theme search form if it exists
	...
	function form( $instance ) {
		$instance = wp_parse_args( (array) $instance, array( 'title' => '' , 'checked_rss' => 'on',  'checked_atom' => 'on', 'checked_feed'=>'on') );
		$title    = $instance['title'];
		$checked_rss    = $instance['checked_rss'];
		$checked_atom    = $instance['checked_atom'];
		$checked_feed    = $instance['checked_feed'];


		?>
	<p style="clear: both"><label for="<?php echo $this->get_field_id( 'title' ); ?>"><?php _e( 'Title:', 'smartadapt' ); ?>
		<input class="widefat"
					 id="<?php echo $this->get_field_id( 'title' ); ?>"
					 name="<?php echo $this->get_field_name( 'title' ); ?>"
					 type="text"
	...
	<p style="clear: both; padding-bottom: 10px;"><label style="width: 200px; float: left" for="<?php echo $this->get_field_id( 'checked_rss' ); ?>"><span style="float: left"><?php _e( 'Show RSS:', 'smartadapt' ); ?></span>
		<input 
					 id="<?php echo $this->get_field_id( 'checked_rss' ); ?>"
					 name="<?php echo $this->get_field_name( 'checked_rss' ); ?>"
					 type="checkbox"
			<?php checked($checked_rss, 'on'); ?> style="float: right" /></label>
	</p>
	<p style="clear: both; padding-bottom: 10px;"><label style="width: 200px; float: left" for="<?php echo $this->get_field_id( 'checked_atom' ); ?>"><span style="float: left"><?php _e( 'Show ATOM:', 'smartadapt' ); ?></span>
		<input 
					 id="<?php echo $this->get_field_id( 'checked_atom' ); ?>"
					 name="<?php echo $this->get_field_name( 'checked_atom' ); ?>"
					 type="checkbox"
			<?php checked($checked_atom, 'on'); ?> style="float: right" /></label>
	</p>
	...
	function update( $new_instance, $old_instance ) {
		$instance          = $old_instance;
		$new_instance      = wp_parse_args( (array) $new_instance, array( 'title' => '', 'checked_rss'=> '','checked_atom'=> '', 'checked_feed'=> ''  ) );
		$instance['title'] = strip_tags( $new_instance['title'] );
		$instance['checked_rss'] = strip_tags( $new_instance['checked_rss'] );
		$instance['checked_atom'] = strip_tags( $new_instance['checked_atom'] );
		$instance['checked_feed'] = strip_tags( $new_instance['checked_feed'] );
		return $instance;
	}

}

add_action( 'widgets_init', create_function( '', 'return register_widget("WP_Widget_Feeds_smartadapt");' ) );
 ...
 * @return void
 */
function smartadapt_add_theme_widgets($old_theme, $WP_theme = null) {
...
smartlib_pre_set_widget( 'sidebar-1', 'widget_feeds',
    array(
        'title' => __('Follow us','smartadapt'),
        'checked_rss' => 'on',
        'checked_atom' => 'on',
        'checked_feed'=>'on',
...
smartlib_pre_set_widget( 'sidebar-1', 'search',
    array(
        'title' => __('Search Form','smartadapt')

    )
);

smartlib_pre_set_widget( 'sidebar-1', 'recent-posts-smartadapt',
    array(
        'title' => __('Last Articles','smartadapt')

    )
);

smartlib_pre_set_widget( 'sidebar-1', 'recent-gallery-widget-smartadapt',
    array(
        'title' => __('Last Galeries','smartadapt'),
         'gallery_limit' => 4,
    )
);
...
add_action('after_switch_theme', 'smartadapt_add_theme_widgets', 10, 2);
 ...
 */
function smartlib_pre_set_widget( $sidebar, $name, $args = array() ) {
