...
function smartadaptpro_function()
{
    ...
    <div class="wrap">
        ...
        <p><a href="<?php echo $pro_link ?>">SmartAdapt Pro</a> adds exciting new customization features to the Theme
            Customizer and other powerful customization tools like shortcodes or layout options. </p>

        <div style="float: left; width: 50%"><p><img src="http://netbiel.pl/smartadaptpro/files/layout-variants.png"
                                                     alt=""></p></div>
            ...
            <div id="submitdiv" class="postbox " style="margin-top: 40px;height: 265px;">
                ...
                <div class="inside">
                    ...
                    <div style="float: left;width: 40%">
                        <img src="http://netbiel.pl/smartadaptpro/wp-content/uploads/2013/11/logo-premium.png"
                             style="max-width: 100%; margin-top: 15px;" alt="SmartAdapt Pro Demo">
                    </div>

                    <a href="http://www.mojo-themes.com/item/smartadapt-pro-clean-responsive-wordpress-theme/"
                       class="button button-primary" style="clear: both; margin-top: 110px" target="_blank"><strong>More
                            info &raquo;</strong></a>
                </div>
...
function my_custom_post_catalogue()
{
    $labels = array(
        'name' => _x('Каталог', 'post type general name'),
        'singular_name' => _x('Каталог', 'post type singular name'),
        'add_new' => _x('Добавить делать', 'book'),
        'add_new_item' => __('Добавить новую деталь'),
        'edit_item' => __('Редактировать деталь'),
        'new_item' => __('Новая деталь'),
        'all_items' => __('Все детали'),
        'view_item' => __('Просмотр деталей'),
        'search_items' => __('Поиск деталей'),
        'not_found' => __('Деталь не найдена'),
        'not_found_in_trash' => __('Деталь не найдена в Корзине'),
        'parent_item_colon' => '',
        'menu_name' => 'Каталог'
    );
    $args = array(
        'labels' => $labels,
        'description' => 'Holds our details and details specific data',
        'public' => true,
        'menu_position' => 5,
...
add_action('init', 'my_custom_post_catalogue');
...
function my_updated_messages( $messages ) {
    global $post, $post_ID;
    $messages['catalogue'] = array(
        0 => '',
        1 => sprintf( __('Catalogue updated. <a href="%s">View catalogue</a>'), esc_url( get_permalink($post_ID) ) ),
        2 => __('Custom field updated.'),
        3 => __('Custom field deleted.'),
        4 => __('Catalogue updated.'),
        5 => isset($_GET['revision']) ? sprintf( __('Catalogue restored to revision from %s'), wp_post_revision_title( (int) $_GET['revision'], false ) ) : false,
        6 => sprintf( __('Catalogue published. <a href="%s">View catalogue</a>'), esc_url( get_permalink($post_ID) ) ),
        7 => __('Catalogue saved.'),
        8 => sprintf( __('Catalogue submitted. <a target="_blank" href="%s">Preview catalogue</a>'), esc_url( add_query_arg( 'preview', 'true', get_permalink($post_ID) ) ) ),
        9 => sprintf( __('Catalogue scheduled for: <strong>%1$s</strong>. <a target="_blank" href="%2$s">Preview catalogue</a>'), date_i18n( __( 'M j, Y @ G:i' ), strtotime( $post->post_date ) ), esc_url( get_permalink($post_ID) ) ),
        10 => sprintf( __('Catalogue draft updated. <a target="_blank" href="%s">Preview catalogue</a>'), esc_url( add_query_arg( 'preview', 'true', get_permalink($post_ID) ) ) ),
    );
    return $messages;
}
...
function my_taxonomies_catalogue() {
    $labels = array(
        'name'              => _x( 'Catalogue Categories', 'taxonomy general name' ),
        'singular_name'     => _x( 'Catalogue Category', 'taxonomy singular name' ),
        'search_items'      => __( 'Search Catalogue Categories' ),
        'all_items'         => __( 'All Catalogue Categories' ),
        'parent_item'       => __( 'Parent Catalogue Category' ),
        'parent_item_colon' => __( 'Parent Catalogue Category:' ),
        'edit_item'         => __( 'Edit Catalogue Category' ),
        'update_item'       => __( 'Update Catalogue Category' ),
        'add_new_item'      => __( 'Add New Catalogue Category' ),
        'new_item_name'     => __( 'New Catalogue Category' ),
        'menu_name'         => __( 'Catalogue Categories' ),
    );
    $args = array(
        'labels' => $labels,
        'hierarchical' => true,
    );
    register_taxonomy( 'catalogue_category', 'catalogue', $args );
}
add_action( 'init', 'my_taxonomies_catalogue', 0 );



//Определение мета поля
add_action( 'add_meta_boxes', 'catalogue_price_box' );
function catalogue_price_box() {
    add_meta_box(                           //Добавляет дополнительные блоки (meta box) на страницы редактирования/создания постов, постоянных страниц или произвольных типов записей в админ-панели.
        'catalogue_price_box',                //id атрибут HTML тега, контейнера блока.
        __( 'Catalogue Price', 'myplugin_textdomain' ),       //Заголовок/название блока. Виден пользователям
        'catalogue_price_box_content',        //Функция, которая выводит на экран HTML содержание блока.
        'catalogue',          //Тип записи для к которой добавляется блок ('post', 'page', 'link', 'attachment' или 'custom_post_type').
...
function catalogue_price_box_content( $post ) {
    wp_nonce_field( plugin_basename( __FILE__ ), 'catalogue_price_box_content_nonce' );                        //wp_nonce_field  Выводит проверочное (защитное, одноразовое) скрытое поле для формы для проверки передаваемых данных формы, чтобы убедиться, что данные были отправлены с текущего сайта
    echo '<label for="catalogue_price"></label>';                                                              // plugin_basename Отрезает из переданного пути до файла или папки плагина, путь до папки плагинов. Оставляет путь от папки плагина до указанного файла плагина.
    echo '<input id="catalogue_price" name="catalogue_price" placeholder="enter a price" type="text">';
}



//Обработка данных( сохранение пользовательских данных )
add_action( 'save_post', 'catalogue_price_box_save' );
function catalogue_price_box_save( $post_id ) {
    if ( defined( 'DOING_AUTOSAVE' ) && DOING_AUTOSAVE )
        return;

    if ( !wp_verify_nonce( $_POST['catalogue_price_box_content_nonce'], plugin_basename( __FILE__ ) ) )          //Проверяет переданный проверочный код.    Параметры: 1)Название поля, которое было передано в форме для проверки;  2) Ключ, который был указан при создании проверочного значения.
        return;
    ...
    update_post_meta( $post_id, 'catalogue_price', $catalogue_price );                  //Обновляет произвольное поле указанного поста или добавляет новое. Параметры: 1)ID поста, произвольное поле которого нужно обновить/создать;
...
function my_get_posts( $query ) {
    if ( ( is_home() && false == $query->query_vars['suppress_filters'] ) || is_feed() )
        $query->set( 'post_type', array( 'post', 'catalogue' ) );
    return $query;
}

// If we are in a loop we can get the post ID easily
//$price = get_post_meta( get_post_ID(), 'catalogue_price', true );

// To get the price of a random catalogue we will need to know the ID
//$price = get_post_meta( $catalogue_id, 'catalogue_price', true );
